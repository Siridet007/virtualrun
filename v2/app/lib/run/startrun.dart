  ...
  void initState() {
    ...
    locationSubscription = _location.onLocationChanged.listen((locationData) {
      ...
      if (lng != _lng && lat != _lat) {
        ...
        http
            .post('${Config.API_URL}/save_position/save',
...
//-------------------------calculate distance---------------------------------//
  ...
  void calculate() {
    ...
    http
        .post(
            '${Config.API_URL}/save_position/show?userId=$myId&id=$allRunId&dateNow=$myDate',
            headers: header,
            body: params)
        .then((res) {
      Map resMap = jsonDecode(res.body) as Map;
      ...
      for (var i = 0; i < _listSum.length - 1; i++) {
        totalDistance += calculateDistance(_listSum[i]["lat"],
            _listSum[i]["lng"], _listSum[i + 1]["lat"], _listSum[i + 1]["lng"]);
      }
